...
def analyze_leaf_image(image_path):
    ...
    try:
        ...
        if st.session_state.model is None:
            # Fallback: Use Gemini AI for disease analysis without ML model
            gemini_engine = get_gemini_engine()
            if gemini_engine:
                st.warning("⚠️ ML model unavailable - using AI-powered analysis instead")
                disease_result = {
                    'disease': 'Unknown (AI Analysis)',
                    'confidence': 0.5
                }
                plant_result = {
                    'plant': 'Unknown',
                ...
                st.error("❌ Model unavailable and AI analysis failed")
                return None
        else:
